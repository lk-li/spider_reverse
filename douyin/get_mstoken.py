...
def get_mstoken(X_Bogus, strData, tspFromClient, user_agent):
...
if __name__ == '__main__':
    ...
    with open('strData.js', 'r', encoding='utf-8') as f:
        strD = f.read()
    strData = execjs.compile(strD).call('getsData', user_agent)

    tspFromClient = int(time.time() * 1000)

    X_Bogus = get_x_bogus(
        "msToken=",
        '{"magic":538969122,"version":1,"dataType":8,"strData":"' + strData + '","tspFromClient":' + str(
            tspFromClient) + '}',
        user_agent
    )
    print(X_Bogus)
    print(tspFromClient)
    get_mstoken(X_Bogus, strData, tspFromClient, user_agent)
