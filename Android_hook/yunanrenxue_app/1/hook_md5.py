...
function hexToBase64(str) {
    return base64Encode(String.fromCharCode.apply(null, str.replace(/|/g, "").replace(/([\da-fA-F]{2}) ?/g, "0x$1 ").replace(/ +$/, "").split(" ")));
}
function base64ToHex(str) {
    for (var i = 0, bin = base64Decode(str.replace(/[ ]+$/, "")), hex = []; i < bin.length; ++i) {
...
'''
# process = frida.get_device_manager().add_remote_device('127.0.0.1:6666').attach('土巴兔装修')
process = frida.get_usb_device(timeout=1000).attach('土巴兔装修')
